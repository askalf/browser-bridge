...
- `browser_click` and `browser_type` use puppeteer's `page.click` / `page.type`, which send CDP
  ...
  `keydown` both read `isTrusted === true`.
- Both wait for the selector to be **visible** first (default 10s, capped at the 45s navigation
  timeout), so an agent does not have to poll. `browser_type` can `clear` the field (triple-click,
  Backspace) and `submit` (Enter), and its result reports the character count, **never the
  text**: it may be a password, and MCP hosts keep tool results in their transcripts. A test
...
- **Trade-off:** `--disable-component-update` stops more than update pings — Chromium ships certificate-revocation data (CRLSet) as a component too, so a long-lived container also stops picking up revocations. For this image's use case, an ephemeral CDP target driven by a trusted client, that's an acceptable trade against the egress it removes. A deployment that keeps one container running for a very long time should weigh that against a periodic restart to pick up a fresh CRLSet.
...
- The match is on the **error code alone**. An earlier revision of this branch also counted any error carrying *no* `code` as unreachable, which quietly inverted the rule above: the only code-less errors reaching that predicate are ones this module synthesises itself, and the 32KB response-head guard is one of them — so an upstream that *answered*, just with an over-long head, tripped the breaker and moved the exit address, at a moment chosen by whatever the upstream sent. Both synthetic connect timeouts tag `code: 'ETIMEDOUT'` deliberately, so nothing legitimate ever needed the catch-all. A test asserts the oversized-head case is a `502` with the breaker still shut.
...
- 11 new tests in `test/proxy-auth-relay.test.mjs` — off-by-default 502, `CONNECT` fallback on refusal, fallback on a silent (black-hole) upstream, `407` *not* failed over, an oversized response head *not* failed over, breaker skipping a dead upstream, breaker staying shut on a healthy one, the plain-HTTP path, `egressStatus()`, and IPv6-bracket handling in `splitHostPort`.
