...
- **Isolated sessions and an MCP endpoint.** `BRIDGE_SESSION_MODE=isolated` gives each connection its own stealth Chromium; `mcp-server.mjs` exposes nine browser tools to any MCP client, and its clicks and keystrokes are trusted input events, not `el.click()`.
